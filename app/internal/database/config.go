...
func LoadAlertConfig() (*models.AlertConfig, error) {
	...
	err := DB.QueryRow(`SELECT enabled, smtp_host, smtp_port, smtp_user, smtp_password, alert_email, from_email,
		COALESCE(status_page_url, ''), COALESCE(smtp_skip_verify, 0), alert_on_down, alert_on_degraded, alert_on_up, COALESCE(alert_on_degraded_recovery, 0),
		...
		&config.AlertOnDown, &config.AlertOnDegraded, &config.AlertOnUp, &config.AlertOnDegradedRecovery,
